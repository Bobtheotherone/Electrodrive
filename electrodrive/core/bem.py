...
def bem_solve(
    ...
    # If cfg.plateau_max is None or absent, plateau-based early stopping
    # is disabled and refinement is controlled solely by max_refine_passes
    # and the target_bc threshold.
    _plateau_raw = getattr(cfg, "plateau_max", None)
    if _plateau_raw is None:
        max_plateau = None
    else:
        try:
            max_plateau = int(_plateau_raw)
        except Exception:
            max_plateau = None
    ...
    try:
        for rp in range(max_refine_passes):
            ...
            if (max_plateau is not None) and plateau >= max_plateau and have_min_panels and have_min_passes:
                logger.info(
