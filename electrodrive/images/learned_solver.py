...
def _group_prox(
    ...
    for gid in unique_groups:
        ...
        lam_val = (
            float(lam_vec[mask].mean().item()) if lam_vec is not None else float(lambda_group)  # type: ignore[arg-type]
        )
        if float(norm_g.detach()) <= lam_val:
            w_out[mask] = 0.0
        else:
            shrink = (norm_g - lam_val) / norm_g
