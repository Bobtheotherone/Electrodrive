...
def _ensure_tile_size(tile_size: Optional[int]) -> int:
    """
    Clamp tile_size to a sensible positive integer.

    Priority:
      1. Explicit tile_size argument if > 0.
      2. Environment variable EDE_BEM_TILE_SIZE if set and valid.
      3. Fallback default of 1024.
    """
    if tile_size is not None and tile_size > 0:
        return int(tile_size)

    env_val = os.getenv("EDE_BEM_TILE_SIZE", None)
...
def _bem_matvec_core_torch(
    ...
    use_laplace_fastpath = isinstance(ker, LaplaceSingleLayerKernel)
    ...
    # Precompute sigma * area (used in every tile)
    sigmaA = sigma * areas  # (N,)

    # Reuse local index template for diagonals
    max_tile = min(tile_size, N)
    local_idx_template = torch.arange(max_tile, device=device)
    ...
    for i0 in range(0, N, tile_size):
        ...
        if use_laplace_fastpath:
            # r2 = ||diff||^2, clamp to avoid singularity
            r2 = (diff * diff).sum(dim=-1).clamp_min(1e-24)  # (T,N)
            # Using rsqrt gives 1/sqrt(r2) in one shot
        ...
        local_idx = local_idx_template[:T]
        ...
        # Use BLAS-backed matvec: (T,N) @ (N,) -> (T,)
        V[i0:i1] = torch.mv(K, sigmaA)
...
def _bem_E_field_targets_core_torch(
    ...
    for i0 in range(0, M, tile_size):
        ...
        if use_laplace_fastpath:
            r2 = (diff * diff).sum(dim=-1).clamp_min(1e-24)  # (T,N)
            inv_r = torch.rsqrt(r2)
            inv_r3 = inv_r * inv_r * inv_r  # (T,N)
            weight = K_E * inv_r3  # (T,N)
        else:
            r = torch.linalg.norm(diff, dim=-1).clamp_min(1e-12)  # (T,N)
            weight = ker.e_field_weight(diff, r)  # (T,N)

        # w_ij = W_ij * (sigma_j * A_j)
        w = weight * sigmaA.unsqueeze(0)  # (T,N)

        # (T,1,N) @ (T,N,3) -> (T,1,3) -> (T,3)
        E[i0:i1] = torch.bmm(w.unsqueeze(1), diff).squeeze(1)
...
def _bem_matvec_keops(
    ...
    V = (K_ij * (1 - is_diag) * SigmaA_j).sum(dim=1).view(N)
    if self_integrals is not None:
...
def bem_matvec_batched(
    ...
    *,
    ...
    # Precompute sigma_j * A_j per batch
    sigmaA = sigma * areas.unsqueeze(0)  # (B,N)
    ...
    for i0 in range(0, N, tile_size):
        ...
        # (B,N) @ (N,T) -> (B,T)
        V[:, i0:i1] = sigmaA @ K.transpose(0, 1)
