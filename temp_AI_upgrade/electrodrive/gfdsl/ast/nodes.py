    ...
    def lower(self, ctx: CompileContext) -> LinearContribution:
        ...
        if self.poles:
            for pole in self.poles:
                if isinstance(pole, InterfacePoleNode):
                    raise NotImplementedError(
                        "Layered evaluator not implemented yet; node lowered as placeholder; implement electrodrive/gfdsl/eval/layered.py"
                    )
                contributions.append(pole.lower(ctx))
        if self.images:
            for img in self.images:
                contributions.append(img.lower(ctx))
        if self.branchcut is not None:
            raise NotImplementedError(
                "Layered evaluator not implemented yet; node lowered as placeholder; implement electrodrive/gfdsl/eval/layered.py"
            )
        ...
        def dense_fn(X: torch.Tensor) -> torch.Tensor:
            cols = [contrib.evaluator.eval_columns(X) for contrib in contributions]
            return torch.cat(cols, dim=1) if len(cols) > 1 else cols[0]
        ...
        if ctx.eval_backend == "operator":
            ...
            def rmatvec_fn(r: torch.Tensor, X: torch.Tensor) -> torch.Tensor:
                grads = [contrib.evaluator.rmatvec(r, X) for contrib in contributions]
