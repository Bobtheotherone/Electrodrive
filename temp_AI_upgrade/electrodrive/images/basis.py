        ...
        if spheres_for_axis:
            ...
            for z_val in z_vals:
                ...
                pos = center + torch.tensor(
                    [0.0, 0.0, float(z_val)], device=device, dtype=dtype
                )
...
class BasisOperator:
    ...
    def __init__(
        ...
        *,
        ...
        self.points = points
        self.device = torch.device(device) if device is not None else points.device
        self.dtype = dtype if dtype is not None else points.dtype

        self.row_weights = None
        self._sqrt_row_weights = None
    ...
    def shape(self) -> Tuple[int, int]:
        return (int(self.points.shape[0]), int(len(self.elements)))

    def _targets(self, targets: Optional[torch.Tensor]) -> torch.Tensor:
        X = self.points if targets is None else targets
