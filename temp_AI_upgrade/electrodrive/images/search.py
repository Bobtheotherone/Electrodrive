    ...
    *,
    ...
    if solver_mode == "lista" and lista_model is not None:
        ...
        try:
            ...
            support_lista = [
                int(i) for i in torch.argsort(torch.abs(w_lista), descending=True).tolist() if float(torch.abs(w_lista[int(i)])) > 0.0
            ]
            if support_k is not None and support_k > 0:
                support_lista = support_lista[:support_k]
            if not lista_refine:
                stats["solver"] = "lista"
                stats["converged"] = True
                w_out, support_out = w_lista, support_lista
            else:
                try:
                    w_refine, support_refine = _ista_call(
                        max_iter_inner=min(max_iter, 120),
                        tol_inner=max(tol, 1e-5),
                        do_refit=False,
                    )

                    def _residual_norm(w_vec: torch.Tensor) -> float:
                        pred = _matvec(A, w_vec, X if isinstance(A, BasisOperator) else None)
                        return float(torch.linalg.norm(pred - V_gt).item())

                    res_lista = _residual_norm(w_lista)
                    res_refine = _residual_norm(w_refine)
                    if res_refine < res_lista:
                        logger.info(
                            "LISTA safeguard: falling back to ISTA refinement.",
                            res_lista=float(res_lista),
                            res_ista=float(res_refine),
                        )
                        if support_refine:
                            stats["solver"] = "lista+ista_refine"
                            w_out, support_out = w_refine, support_refine
                        else:
                            w_out, support_out = _ista_call(
                                max_iter_inner=max_iter,
                                tol_inner=tol,
                                do_refit=ls_refit,
                            )
                    ...
                    logger.warning(
                        "LISTA refinement failed; using raw LISTA output.",
                        error=str(exc),
                    )
                    if support_lista:
                        stats["solver"] = "lista"
    ...
    try:
        ...
        resid = preds - V_gt
        bc_norm = float(torch.linalg.norm(resid[is_boundary]) if is_boundary is not None and bool(is_boundary.any()) else torch.linalg.norm(resid))
        stats["bc_norm"] = bc_norm
        if is_boundary is not None and bool(is_boundary.any()):
            interior_mask = ~is_boundary
            stats["int_norm"] = float(torch.linalg.norm(resid[interior_mask])) if bool(interior_mask.any()) else float("nan")
...
def discover_images(
    ...
    if two_stage:
        ...
        w_out = weights_full
        support_full = [int(i) for i in torch.argsort(torch.abs(w_out), descending=True).tolist() if float(torch.abs(w_out[int(i)])) > 0.0]
