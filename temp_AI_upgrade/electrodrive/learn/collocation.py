...
def _evaluate_oracle_on_points(
    ...
    Note
    ----
    Both analytic shortcuts and BEM oracles return SI-scale potentials
    using the standard Coulomb constant K_E = 1 / (4π ε₀). This keeps the
    collocation targets on a consistent scale regardless of which oracle
    is used.
    """
    ...
    if isinstance(solution, AnalyticSolution):
        ...
        # Return SI-scale potentials so analytic and BEM oracles remain
        # consistent. Downstream callers can still rescale if needed.
        return V_gt
