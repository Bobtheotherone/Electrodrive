...
# tests. Keep fp64 + near-field corrections for accuracy, but allow early exit
# after a single refinement pass to keep runtime manageable on laptops.
BEM_TEST_ORACLE_CONFIG = {
    ...
    "use_gpu": True,
    # Start finer and allow up to two passes while permitting early exit if
    # the BCs are already accurate enough.
    "initial_h": 0.2,
    "min_refine_passes": 1,
    "max_refine_passes": 2,
    "refine_factor": 0.8,
...
def test_analytic_matches_bem_up_to_units(spec_builder, geom_type, needs_eps_scaling):
    ...
    Unlike the original "slow / opt-in" semantics, this test now treats
    BEM as a hard dependency: if BEM is unavailable or fails to converge,
    the test fails, forcing us to fix the backend rather than skipping.
    """
    # BEM must be available for this test to be meaningful.
    assert BEM_AVAILABLE, (
        "BEM backend must be available for analytic-vs-BEM sanity checks. "
        "Install/build the BEM dependencies and ensure BEM_AVAILABLE=True."
    )
